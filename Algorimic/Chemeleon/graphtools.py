...
def knn_graph(df, k, verbose=False):
    ...
    points = [p[1:] for p in df.itertuples()] # take samples
    g = nx.Graph()
    
    # nodes = index of each points in points
...
def pre_part_graph(graph, k, df=None, verbose=False):
    ...
    clusters = 0
    for i, p in enumerate(graph.nodes()): # p = node index
        graph._node[p]['cluster'] = 0 # intial lize
    cnts = {}
    cnts[0] = len(graph.nodes()) # {0: 788}

    while clusters < k - 1: # choose one cluseters two split
        ...
        s_nodes = [n for n in graph._node if graph._node[n]['cluster'] == maxc] # take all node in max cluster, n = {pos, cluster}
        s_graph = graph.subgraph(s_nodes) # including points and edge
